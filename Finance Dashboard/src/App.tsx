...
function App() {
  ...
  const renderMobileDashboard = () => (
    <section className="mobile-page">
      ...
      <p className="label muted">THIS MONTH TAKE-HOME</p>
      <div className="mobile-admin-row">
        <h1>{formatInr(monthlyTakeHome)}</h1>
        <button className="switch" onClick={() => setIsAdmin((prev) => !prev)}>
          <span>{isAdmin ? 'ADMIN MODE' : 'VIEWER MODE'}</span>
          <i className={isAdmin ? 'active' : ''} />
        </button>
      </div>
      <p className="green">+8.2% VS LAST MONTH</p>

      <article className="card mobile-balance-card">
        <div className="card-head">
          <div>
            <h3>Cash Flow Trend</h3>
            <p>Last 6 months income vs expenses</p>
          </div>
          <span className="chip">H1 2024</span>
        </div>
        <div className="bar-graph small">
          {monthBars.map((bar, i) => (
            <div key={bar + i} style={{ height: `${bar}%` }} />
          ))}
        </div>
      </article>

      <div className="cards two mobile-kpi">
        <article className="card center-kpi">
        ...
        <article className="card center-kpi left">
          <div className="metric-icon metric-cyan">
            <CreditCard size={18} />
          </div>
          <p>SALARY CREDIT</p>
          <h3>{formatInr(salaryIncome || 275000)}</h3>
          <span className="green">Primary income</span>
        </article>
      </div>

      <p className="label mobile-section-label">SALARY SPLIT</p>
      ...
      <article className="card list-item">
        ...
        <div>
          <h3>Fixed Costs</h3>
          <p>Rent + EMI + insurance</p>
        </div>
        <div className="right-val">
          <strong>{formatInr(fixedCosts || 134500)}</strong>
          <span className="muted">Needs 50% cap</span>
        </div>
      </article>
      ...
      <div className="inline-actions full">
        ...
        <button className="secondary" onClick={() => exportData('csv')}>
          Export
        </button>
      </div>
    </section>
  ...
  const renderMobileInsights = () => (
    <section className="mobile-page">
      ...
      <p className="label muted">SALARIED HEALTH SCORE</p>
      <div className="split-line mobile-velocity">
        <h2>Savings Discipline</h2>
        <strong>44%</strong>
      </div>
      ...
      <p>
        Your salary usage is stable; increase SIP and emergency-fund transfers this month to
      ...
      <article className="card growth-card enhanced-growth">
        <div className="growth-header">
          <div>
            <h3>Salary Allocation Snapshot</h3>
            <p className="muted">Monthly salary distribution</p>
          </div>
          <span className="chip green-glow">Spending in control</span>
        </div>
        <h2 className="growth-value">{formatInr(monthlyTakeHome)}</h2>
        <p className="growth-desc">Expected month-end surplus</p>
        <div className="allocation-bars mobile">
          <div className="allocation-item compact">
            <span className="bar liquid" style={{ width: '62%' }} />
            <span className="alloc-name">Essentials: {formatInr(fixedCosts || 134500)}</span>
          </div>
          <div className="allocation-item compact">
            <span className="bar market" style={{ width: '38%' }} />
            <span className="alloc-name">Savings + SIP: {formatInr(monthlyTakeHome)}</span>
          </div>
        </div>
      </article>

      <div className="mobile-cash-head">
        <p className="label muted">CASH FLOW ANALYSIS</p>
        <div className="segmented compact">
          {(['q1', 'q2', 'q3', 'q4'] as const).map((q) => (
            <button
              key={q}
      ...
      <article className="card chart-card mobile-chart">
        <div className="mini-bars">
          {insights.income.map((point, idx) => (
            <div key={point + idx} className="pair">
              <div className="stick-slot">
                <span style={{ height: `${point * 24}px` }} className="stick income" />
                <div className="bar-tooltip-card">
                  <p>{insightMonths[idx]} Income</p>
                  <strong>{formatInr(point * 1000000)}</strong>
                </div>
              ...
              <div className="stick-slot">
                <span style={{ height: `${insights.expense[idx] * 24}px` }} className="stick expense" />
                <div className="bar-tooltip-card">
                  <p>{insightMonths[idx]} Expense</p>
                  <strong>{formatInr(insights.expense[idx] * 1000000)}</strong>
                </div>
      ...
      <p className="label muted">CATEGORY INSIGHTS</p>
      <article className="card list-item">
        <div className="metric-icon metric-purple">
          <HandCoins size={16} />
        </div>
        <div>
          <h3>Food & Dining</h3>
          <span className="red">+12% vs last month</span>
        </div>
        <div className="right-val">
          <strong>₹850</strong>
          <span>/ mo</span>
      ...
      <article className="card list-item">
        ...
        <div>
          <h3>Wealth Accrual</h3>
          <span className="green">On Target</span>
        </div>
        <div className="right-val">
          <strong>45%</strong>
        </div>
      </article>
      <article className="card list-item alert">
        <div className="metric-icon metric-rose">
          <Home size={16} />
        </div>
        <div>
    ...
    <section className="mobile-page">
      ...
      <div className="mobile-filters">
        <button
          ...
          onClick={() =>
            setCategory((prev) => {
              ...
              return allCategories[(idx + 1) % allCategories.length] as Category | 'All'
            })
          }
        >
        ...
        <button
          className="secondary"
          onClick={() =>
            setKind((prev) => (prev === 'all' ? 'income' : prev === 'income' ? 'expense' : 'all'))
          }
        >
        ...
        <button
          className="secondary"
          onClick={() =>
            setRange((prev) => (prev === '30d' ? '90d' : prev === '90d' ? '365d' : '30d'))
          }
        >
      ...
      <div className="mobile-list-head">
        ...
        <button className="ghost" onClick={() => exportData('csv')}>
          View Reports
        </button>
      </div>

      {filtered.slice(0, 4).map((tx) => (
        <article key={tx.id} className="card list-item mobile-tx">
          <div className={`metric-icon ${tx.type === 'income' ? 'metric-cyan' : 'metric-purple'}`}>
            {tx.type === 'income' ? <Building2 size={16} /> : <CreditCard size={16} />}
          </div>
          <div>
            <h3>{tx.description.split(' - ')[0]}</h3>
            <p>
              {formatDate(tx.date)} • <span className="chip">{tx.category.toUpperCase()}</span>
            </p>
          </div>
          <div className="right-val">
            <strong className={tx.type === 'income' ? 'green' : 'red'}>
              {tx.type === 'income' ? '+' : '-'}
              {formatInr(tx.amount)}
            </strong>
            <span>{tx.status}</span>
      ...
      <article className="card volume-card">
        <p className="label">EXPECTED MONTHLY SPEND</p>
        <h2>{formatInr(monthVolume)}</h2>
      </article>

      <div className="cards two mobile-kpi">
        <article className="card center-kpi left">
          <p className="label muted">NET CASH FLOW</p>
          <h3 className="green">+12.4%</h3>
        </article>
        <article className="card center-kpi left">
          <p className="label muted">TAX PROVISION</p>
          <h3>{formatInr(8400)}</h3>
        </article>
      </div>
    </section>
  )
